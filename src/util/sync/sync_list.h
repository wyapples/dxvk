...
// 
// I do not entirely understand the purpose of loop in List<>::insertEntry, but it looks
// like it could cause CPU overhead if pipelines are constantly re-inserted.
#ifndef GTR2_SPECIFIC
  template<typename T>
  class List
  {
  ...
  public:
    ...
    public:
      ...
      reference operator*() const { return m_entry->data; }

      pointer operator->() const { return &m_entry->data; }
    ...
    ~List() { freeList(m_head.load()); }

    auto begin() const { return Iterator(m_head); }
    auto end() const { return Iterator(nullptr); }

    Iterator insert(const T& data) { return insertEntry(new Entry(data)); }

    Iterator insert(T&& data) { return insertEntry(new Entry(std::move(data))); }
  ...
  private:
    ...
    Iterator insertEntry(Entry* e)
    {
      ...
      do {
        ...
        next, e, std::memory_order_release, std::memory_order_acquire));
